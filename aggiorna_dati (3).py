...
def esegui_query(endpoint, query):
...
def interroga_fonte(fonte):
    ...
    try:
        prova = esegui_query(fonte["endpoint"], fonte["vitale"])
    ...
    try:
        righe = esegui_query(fonte["endpoint"], fonte["query"])
    ...
    return "OK", "%d atti" % len(atti), atti
